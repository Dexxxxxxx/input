 ...
 * 
 */
define([ 'jquery', 'app', 'ztree', 'jqueryForm','My97DatePicker','text!biz/cigarette/gencigarette.html' ], function($, app, ztree, jqueryForm,My97DatePicker,tpl) {
	var page=10;
	var showFpType='';//点击单张开票时刷新发票列表 0=批量 1=单条数据
	//全局加载页面
	var loadData = function(){
		new app.Page({
			url : app.rootPath + '/cigarette/gencigarette/list.do?type=2',
			target : 'page-div',
			from: 'search-form',
			limit : page,
		...
		app.ajax({
			...
			success:function(data){
				...
				$.each(data,function(i,item){
					html += ''
						+'<tr>'
						+'	<td style="width: 10%;">'+item['SPXH']+'</td>'
						+'	<td style="width: 20%;">'+item['SPMC']+'</td>'
...
//点击开票获取待开发票信息
		$('#btn-FpInfo').click(function(){
			...
			if(ids==''){
				...
				$('.fiex').show();
				$.ajax({
					url:app.rootPath + '/cigarette/gencigarette/doInvoice.do',
					type:'post',
					dataType:'json',
					data:{'ids':app.ids},
					success:function(data){
...
//单张开具主方法（普票）
		function doInvoiceByOne(invoice_id){
			$('.fiex').show();
			app.ajax({
				url:app.rootPath + '/cigarette/gencigarette/doInvoiceByOne.do',
				type:'post',
				dataType:'json',
				data:{'id':invoice_id},
				success:function(data){
...
//修改客户信息
		...
		var showFpInfo = function(invoice_no,order_no){
			...
			app.ajax({
				url:app.rootPath + '/cigarette/gencigarette/getInvoiceList.do',
				type:'POST',
