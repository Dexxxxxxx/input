 ...
 * 
 */
define([ 'jquery', 'app', 'ztree', 'jqueryForm','My97DatePicker','text!biz/cigarette/ecigarette.html' ], function($, app, ztree, jqueryForm,My97DatePicker,tpl) {
	var page=10;
	var showFpType='';//点击单张开票时刷新发票列表 0=批量 1=单条数据
	//全局加载页面
	var loadData = function(){
		new app.Page({
			url : app.rootPath + '/cigarette/ecigarette/list.do?type=2',
			target : 'page-div',
			from: 'search-form',
			limit : page,
	...
	var loadData_items=function(){
		app.ajax({
			url:app.rootPath + '/cigarette/ecigarette/getById.do?invoice_no='+app.pid,
			type:'POST',
...
//点击开票获取待开发票信息
		$('#btn-FpInfo').click(function(){
			...
			if(ids==''){
				...
				$('.fiex').show();
				$.ajax({
					url:app.rootPath + '/cigarette/ecigarette/doInvoice.do',
					type:'post',
					dataType:'json',
					data:{'ids':app.ids},
					success:function(data){
...
//行事件
		$('#data-tbody-FpInfo').on('click', 'button', function(){
			...
			if(btn=='doInvoiceOne'){
				doEInvoiceByOne(invoice_id);
			}
		});
		//单张开具主方法（电票）
		function doEInvoiceByOne(invoice_id){
			$('.fiex').show();
			app.ajax({				
				url:app.rootPath + '/cigarette/ecigarette/doInvoiceByOne.do',
				type:'post',
				dataType:'json',
				data:{'id':invoice_id},
				success:function(data){
...
//修改客户信息
		$('#data-tbody').on('click','button[name="edit"]',function(e){
			//修改客户信息
			e.stopPropagation();
...
//2、查余票
		//查余票返回校验
//3、执行开票(单张、多张)
		var doInvoice = function(invoice_id,xml){
			ret=xmlCheck(xml);
			saveInvoice(invoice_id,ret);
		}
		//执行开票完成、保存发票信息
		//7、模态显示(发票开具情况)
		var showFpInfo = function(invoice_no,order_no){
			...
			app.ajax({
				url:app.rootPath + '/cigarette/ecigarette/getInvoiceList.do',
				type:'POST',
				...
				success:function(data){
					var html = ''; 
					$.each(data,function(i,item){
						var invoice_type=item['TYPE']=='007'?'普票':'电子发票';
						var buttn='';
							if(item['STATUS']=='0'){
								buttn += ''
									+'	<td>'
